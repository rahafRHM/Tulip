...
<div class="flex-container" dir="rtl">
  <?php
  $sql="SELECT * FROM shoppingCart WHERE Section='FoodSupplements'";
  $stmt = $conn->prepare($sql);
    			$stmt->execute();
    			$result = $stmt->get_result();
    			while ($row = $result->fetch_assoc()):
   ?> <a href='ProductDetails.php?productID="<?=$row['productID']?>"' class="productCardLink"><div class="productCard" >
     <div class="productimgDiv">
       <img src="<?= $row['img_path'] ?>" alt="" class="productimg" id="productimg">

     </div>
  <div class="productdetails">
    <p><?=$row['title']?></p>
    <h4 class="pricerrp">JD<?= $row['price'] ?>
      <?php if ($row['rrp'] > 0): ?>
      <span class="rrp">JD<?=$row['rrp']?></span>
      <?php endif; ?> </h4>
